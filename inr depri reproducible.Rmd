...
```{r}
#How India’s monthly trade deficit correlates
#with INR/USD depreciation using your dataset.
deficit_df<- combined%>%
  arrange(date)%>%
  mutate(exchange_dep = c(NA, diff(log(avg_exchange_rate))))%>%
  select(date, exchange_dep, deficit_usd_bn)%>%
  na.omit()
deficit_cor<-cor(deficit_df$deficit_usd_bn, deficit_df$exchange_dep)# we've got slight negative correlation 
#visualization
...
ggplot(data = fpi_df, aes(x = date)) +
  ...
  # Highlight point during severe COVID depreciation
...
```{r}
#Asymmetry: Does depreciation respond more to repo hikes than
#to repo cuts (vs. fed changes)
repofed_df<- combined%>%
  mutate(exchange_dep = c(NA, diff(log(avg_exchange_rate))),
...
```{r}
#Is INR depreciation associated with higher CPI inflation in subsequent months (pass-through effect)?
infcpi_df<- combined%>%
...
#based on results:
#Conclusion: INR volatility is more influenced by global risk sentiment
#(US VIX) than domestic uncertainty (India VIX).
...
```{r}
#Do INR depreciation during high VIX periods reverse quickly, or do they persist?
...
```{r}
#When India’s GDP growth outperforms the US, 
#does the INR strengthen, or is the effect negligible?
diff_gdp<- gdp%>%
...
#While India's GDP growth outperforming the US might theoretically lead to a stronger INR,
#the observed correlation of +0.21 indicates a weak and slightly opposite effect —
#suggesting exchange rates are likely driven more
#by other factors such as interest rate differentials, global risk sentiment,
#and capital flows, rather than relative GDP growth alone.
